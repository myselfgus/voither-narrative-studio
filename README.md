...
```javascript
// Sample JSON structure (upload this)
{
  "metadata": {
    "paciente_id": "GAB-001",
    "contexto": "Análise Narrativa - Retirada de Medicamento",
    "data_analise": "2025-01-15",
    "medico_responsavel": "Dr. Gustavo Mendes e Silva",
    "crm": "CRM 218133/SP"
  },
  "reportTitle": "Relatório Narrativo Inicial",
  "keyQuote": "Parece o desenho na água, tá ligado?",
  "sections": [
    {
      "title": "Introdução Clínica",
      "intro": [{ "type": "paragraph", "content": "O paciente apresenta..." }],
      "subsections": [{ "title": "Histórico", "blocks": [...] }]
    }
  ]
}
```

The LLM will enrich missing fields, generate narrative content, and structure for PDF output.
...
- `worker/`: Cloudflare Workers backend (agent, routes, tools, MCP integration).
- `src/components/ui/`: shadcn/ui primitives (do not modify).
- Key files:
  - `src/pages/HomePage.tsx`: Main builder UI (upload, editor, preview).
  - `worker/agent.ts`: ChatAgent for LLM orchestration.
  - `worker/userRoutes.ts`: Custom API routes (extend here).

### Running Tests

No E2E tests by default. Add with Vitest:
```
bun add -D vitest
bun test
```
...
Extend `worker/chat.ts` to modify system prompts for clinical enrichment. Ensure prompts guide the LLM to output structured JSON matching `NarrativeReportData` interface.

### Adding Features

- New tools: Extend `worker/tools.ts` and register in MCP.
- UI components: Use shadcn/ui; import from `@/components/ui/*`.
- State: Use Zustand slices (primitive selectors only to avoid re-renders).
...
- `CF_AI_API_KEY`: Securely stored server-side.
