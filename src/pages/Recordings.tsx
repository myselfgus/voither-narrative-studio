  ...
  const handleUpload = async () => {
    ...
    reader.onloadend = async () => {
      ...
      if (sessionRes.success && sessionRes.data) {
        ...
        const newRecording = {
          url: base64data,
          duration: audioBlob.size / 6000, // Approximation
          timestamp: Date.now(),
        };
    ...
    <AppLayout>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            <h1 className="font-display font-bold text-4xl text-text-primary">Record Audio</h1>
            <p className="text-muted-foreground">Capture audio and attach it to a patient session.</p>
          </div>
        </div>
        <Card>
          <CardHeader>
            <CardTitle>New Recording</CardTitle>
            <CardDescription>Select a session and start recording. The audio will be saved in high-quality WEBM format.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div>
              <label className="text-sm font-medium mb-2 block">Select Session</label>
              <Select onValueChange={setSelectedSessionId} disabled={recordingState !== 'idle'}>
            ...
            <div className="p-4 border rounded-lg bg-surface-subtle">
              <canvas ref={waveformCanvasRef} width="600" height="100" className="w-full h-[100px] rounded"></canvas>
            </div>
            {audioUrl && (
              <div className="flex items-center gap-4">
                ...
                <Button size="icon" onClick={() => audioRef.current?.play()} disabled={recordingState === 'playing'}>
                  <Play className="w-4 h-4" />
                </Button>
                <Button size="icon" onClick={() => audioRef.current?.pause()} disabled={recordingState !== 'playing'}>
                  <Pause className="w-4 h-4" />
                </Button>
                <div className="flex-grow text-sm text-muted-foreground">
                  Duration: {audioBlob ? (audioBlob.size / 6000).toFixed(2) : 0}s
                </div>
              </div>
            )}
            <div className="flex flex-col sm:flex-row gap-4">
              {recordingState === 'idle' && (
                <Button onClick={startRecording} disabled={!selectedSessionId} className="w-full sm:w-auto">
              ...
              {recordingState === 'stopped' && (
                <>
                  <Button onClick={handleUpload} className="w-full sm:w-auto">
                    <UploadCloud className="w-4 h-4 mr-2" /> Save to Session
                  </Button>
                  <Button onClick={resetRecording} variant="outline" className="w-full sm:w-auto">
