...
export const Header = ({ patientId, date }: { patientId: string; date: string }) => (
  <header className="border-b border-border pb-4 mb-12 flex justify-between items-end print:mb-8 print-break-inside-avoid">
    ...
    <div className="text-right">
      <MonoMeta>{patientId} • {(() => { const d = new Date(date); return isNaN(d.getTime()) ? String(date || '') : d.toLocaleDateString('pt-BR', { timeZone: 'UTC' }); })()}</MonoMeta>
    </div>
  </header>
);
export const Footer = ({ doctor, crm }: { doctor: string; crm: string }) => (
      ...
      <div className="col-span-2 flex justify-between items-end mt-2">
        <MonoMeta>
          {(() => {
            ...
            return isNaN(d.getTime()) ? String(data.metadata.data_analise || '') : d.toLocaleDateString('pt-BR', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' });
          })()}
        </MonoMeta>
      </div>
    </div>
